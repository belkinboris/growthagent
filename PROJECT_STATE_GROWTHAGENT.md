# Состояние проекта (исторический снимок)

Настоящее содержимое этого файла было утеряно 23.07.2026 при загрузке файлов
через веб-интерфейс GitHub. 28.07.2026 (задача C9) выяснилось, куда оно
делось: текст лежал в `SPEC_TRUEPOST_QUEUE_OFFER.md` — та же порча просто
перемешала файлы местами. Здесь он восстановлен целиком, как был.

**Это снимок состояния на 23.07.2026, а не текущая правда.** Актуальные
документы:

- **`PRODUCT_ROADMAP.md`** — принципы продукта, бэклог и журнал работ
  (что сделано, как проверено, что осталось);
- **`CLAUDE.md`** — устройство репозитория, команды запуска и проверки,
  соглашения;
- **`CONTRACT.md`** — что продукт должен отдавать аналитику.

Снимок оставлен потому, что в нём есть то, чего больше нигде нет: причины
старых решений и разбор прод-инцидентов (в частности, OOM на Railway
16.07.2026 и почему granular-отчёты Директа качаются последовательно).

---

## Снимок PROJECT_STATE на 23.07.2026
