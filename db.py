...
async def run_cycle_once(project_id: int | None = None) -> CycleResult:
    ...
    with get_session() as session:
        ...
        if direct_configured and should_run_deep_diagnostics(result.primary_candidate):
            ...
            if cached is not None:
                ...
                if diag_result is not None:
                    ...
                    _log_memory("after deep diagnostics")
                else:
        ...
        # Гигиена памяти (инцидент OOM): большие промежуточные структуры
        # (TSV Директа, journeys/events, дифф-снапшоты) к этому моменту вышли
        # из скоупа -- принудительный сбор возвращает память ОС заметно
        # раньше, а замер фиксирует базовый уровень между циклами.
        import gc
        gc.collect()
        _log_memory("cycle end")
        return result
...
async def run_direct_intelligence_for_project(
    ...
    _log_memory("before query classification")
...
async def run_deep_diagnostics_for_project(project: Project, settings, period_hours: int):
    ...
    # ФИКС OOM 2026-07-17: раньше оба granular-отчёта качались и парсились
    # ПАРАЛЛЕЛЬНО (asyncio.gather) -- в пике памяти висели одновременно два
    # сырых TSV + два разобранных списка dict'ов. По логам падения процесс
    # убивался ровно на этом шаге (два "200 OK" подряд -> рестарт без
    # traceback = OOM-kill). Последовательно: пик вдвое ниже, каждый TSV
    # освобождается до скачивания следующего. Цена -- пара лишних секунд
    # раз в 3 часа, для фонового агента несущественно.
    for coro in (_fetch_ad_groups(), _fetch_queries()):
        report_name, report, error = await coro
        ...
        _log_memory(f"after granular report: {report_name}")
