...
def assess(weekly_points: list, finished_experiments: int = 0) -> Course:
    ...
    if course.registrations_trend == "мало данных" and max(regs_clean or [0]) < TINY_WEEKLY:
        ...
        course.action = (
            ...
            "другая формулировка боли в объявлении). Заодно спросим тех, кто "
            "продукт попробовал и не вернулся, — вопрос им задаст сам продукт."
        )
        ...
        course.action = (
            "Самый быстрый путь к правде — не новая функция, а слова тех, кто "
            "не заплатил. Ходить и спрашивать не нужно: вопрос задаст сам "
            "продукт, ответы придут на вкладку Продакта. Никакая метрика их "
            "не заменит — причину неоплаты знают только сами люди."
        )
    else:
        ...
        course.action = (
            ...
            "рост не даёт. Нужна одна гипотеза уровня продукта (не кнопки): "
            "другая аудитория, другая цена или другое обещание на первом "
            "экране. Чтобы выбрать её не наугад, спросим людей — вопрос "
            "задаст сам продукт."
        )
