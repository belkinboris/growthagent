...
## Вопросы вашим людям (необязательно, но это путь к PMF)

Причину, по которой человек посмотрел цены и не заплатил, не видно ни в
одной метрике — её знает только он сам. Раньше это заканчивалось советом
владельцу «сходите и поговорите с пятью людьми»; теперь спрашивает сам
продукт, а аналитик формулирует вопрос и собирает ответы.

Направление намеренно обратное остальным разделам: **продукт забирает
вопрос**, а не платформа рассылает. У аналитика нет и не должно быть
доступа к вашим людям — наружу отдаются только анонимные ключи. Кто такой
`u_febdae54` в телеграме, знает только ваш продукт.

Оба вызова используют **тот же токен**, что и отметки о выкатке (кнопка
«Выпустить токен» на вкладке «Проекты»), в заголовке
`Authorization: Bearer <токен>`.

### `GET /growth/api/public/projects/{project_id}/questions`

```json
{"ok": true, "questions": [
  {"id": 7, "segment": "pricing",
   "question": "Вы посмотрели тарифы, но не оформили подписку. Подскажите, пожалуйста, что вас остановило?",
   "answers_collected": 2, "answers_needed": 5}
]}
```

`segment` — значение `stuck_at` из `user-journeys`: кому задавать вопрос.
Кто из ваших людей ему соответствует, решаете вы. Пустой список — значит
спрашивать сейчас нечего (так будет большую часть времени: вопрос
появляется, только когда есть что выяснять).

Когда ответов набрано достаточно, вопрос исчезает из списка сам —
специально останавливать опрос не нужно.

### `POST /growth/api/public/projects/{project_id}/answers`

```json
{"question_id": 7, "user_key": "u_febdae54", "text": "дорого для одного канала"}
```

`user_key` — тот же анонимный ключ, что в `user-journeys`. По нему один
человек не засчитывается дважды; повторный ответ вернёт
`{"ok": true, "duplicate": true}` и ничего не сохранит. Текст сохраняется
как написан — ценность именно в формулировках людей.
